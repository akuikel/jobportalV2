...
public class JobPostActivityController {
    ...
    public String globalSearch(Model model,
        ...
        if (!dateSearchFlag && !remote && !type && !StringUtils.hasText(job) && !StringUtils.hasText(location)) {
            ...
            java.util.List<String> typeList = new java.util.ArrayList<>(Arrays.asList(partTime, fullTime, freelance));
            java.util.List<String> remoteList = new java.util.ArrayList<>(Arrays.asList(remoteOnly, officeOnly, partialRemote));

            if (remote == false) { // remote boolean tracks job type filters (naming confusion in original code)
                typeList.add("");
            }
            if (type == false) { // type boolean tracks remote filters
                remoteList.add("");
            }

            jobPost = jobPostActivityService.search(job, location, typeList, remoteList, searchDate);
        }
